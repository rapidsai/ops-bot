...
describe("Release Drafter", () => {
  ...
  test("doesn't run on invalid version branches", async () => {
    mockedAxios.get.mockResolvedValue({
      data: {
        stable: { version: "21.04" },
        nightly: { version: "21.06" },
        legacy: { version: "21.02" },
      },
    });
    await new ReleaseDrafter(
      makePushContext({
        ref: "branch-0.13",
  ...
  test.each([
    ...
    expect(mockGetReleaseByTag.mock.calls[0][0].tag).toBe(
      `v${getVersionFromBranch(branch.payload.ref)}.00a`
    );
    expect(mockCreateRelease).not.toHaveBeenCalled();
