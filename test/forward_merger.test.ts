...
describe("Forward Merger", () => {
  ...
  test("mergeForward should sort branches", async () => {
    ...
    const mockSortBranches = jest
      .fn()
      .mockName("sortBranches")
      .mockReturnValue(sortedBranches);
  ...
  test("mergeForward should merge PR after opening PR", async () => {
    ...
    const mockMergePR = jest
      .fn()
      .mockName("mergePR")
      .mockResolvedValue({ merged: true });
    forwardMerger.context.octokit.pulls.merge = <any>mockMergePR;
    forwardMerger.issueComment = jest
      .fn()
  ...
  test("should not merge PR if there is no PR", async () => {
    ...
    forwardMerger.getNextBranch = jest
      ...
      .mockReturnValue(nextBranch);
    forwardMerger.context.octokit.pulls.create = <any>(
      jest.fn().mockName("openPR").mockResolvedValue({ data: {} })
    );
